 ...
 * @return object
 */
function wechatInfo($url = '')
{
    if (empty($url)) {
        $url = $_SERVER['HTTP_HOST'];
    }
    $wechat = new common\models\RingWechat;
    if($url == 'www.ningyong195.com') {
        $url = 'www.167china.net';
    }
    $data = $wechat::find()->where(['url' => $url])->one();
    if (!empty($data)) {
        return (object)$data->attributes;
    }
    return (object)$wechat::find()->orderBy('admin_id ASC')->one()->attributes;
}
