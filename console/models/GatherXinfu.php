...
class GatherXinfu extends Gather
{
    ...
    public function run()
    {
        ...
        foreach ($this->productList as $tableName => $info) {
            if (is_int($tableName)) {
                $start = strtotime(date('Y-m-d 00:00:00', $nowtime));
                if ($info['trade_time']) {
                ...
                if (empty($param = session('initDataParam' . $info['table_name']))) {
                    ...
                    session('initDataParam' . $info['table_name'], $productParam->attributes, 7200);
                }
                ...
                if ($result) {
					...
					if (in_array($tableName, ['usd'])) {
						$price = $price * 1000;
                        $data->now = $data->now * 1000;
